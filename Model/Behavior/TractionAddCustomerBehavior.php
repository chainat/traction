...
//require(dirname(__FILE__) . '/../../Vendor/tractionAddCustomer.inc.php');


class TractionAddCustomerBehavior extends ModelBehavior {
	protected $_defaults = array(
			"traction_url"					=> 	"http://api.tractionplatform.com/ext/",				// traction live api base url
			"traction_addCustomer_url"		=> 	"AddCustomer",										// multisubscribe api uri			
	...
	private function getTractionObject(Model $Model, $tractionData){
		...
		$traction_url = $this->settings[$Model->alias]['traction_url'];
		$traction_addCustomer_url = $this->settings[$Model->alias]['traction_addCustomer_url'];
		
		$addCustomerObj = new tractionAddCustomer($this->settings[$Model->alias]['traction_url'] . $this->settings[$Model->alias]['traction_addCustomer_url']);
		
		
		$addCustomerObj->addCustomer(
		...
		$addCustomerObj->setLoginDetails($this->settings[$Model->alias]['traction_uid'], $this->settings[$Model->alias]['traction_pwd'], $this->settings[$Model->alias]['traction_ep']);
		$addCustomerObj->setUser("email", $tractionData['email']);
		$addCustomerObj->setReplyUrl($this->settings[$Model->alias]['reply_url']);
	 ...
	 */
	public function tractionAddCustomer(Model $Model, $data){
		// Prepare data
		$tractionData = $this->mapData($Model, $data);
		
		// Get object
		$addCustomerObj = $this->getTractionObject($Model, $tractionData);		
		
		// Make a request
		$result = $addCustomerObj->sendToTraction();
		//debug($result);
		...
		if (!$myResult[0]) {
			...
			if ($myResponseHeaders["RESULT"] == 0) {
				...
				if ($myResponseHeaders["RESULT"] == 999) {
					...
					$msg = "Please check all form field values.";
				}
			}
		}
